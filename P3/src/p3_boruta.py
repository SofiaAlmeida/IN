...
def plotImp(model, features , num = 20):
    ...
    plt.tight_layout()
    plt.savefig('../fig/lgbm_importances_' + sys.argv[0][-5:-3] + '.png')
...
def validacion_cruzada(modelo, X, y, cv):
    ...
    for train, test in cv.split(X, y):
        t = time.time()
        modelo = feat_selector.fit(X[train],y[train])
        tiempo = time.time() - t
...
def preprocessing(data_x, y, data_x_tst):
    '''
...
'''
    ...
    data_x_tmp = data_x_tmp.values
    ''' Configuramos BorutaPy para la selección de características en función de la configuración hecha para Random Forest
    '''
    rf = RandomForestClassifier(n_jobs=-1, class_weight='balanced_subsample', max_depth=5)

    feat_selector = BorutaPy(rf, n_estimators=100, verbose=2, random_state=123456)
   # all_features = np.array(list(data_x_tmp)[0:-1])
    #print(all_features)
    t = time.time()
    #Lo aplicamos sobre nuestros datos.
    feat_selector.fit(data_x_tmp, y)
    tiempo = time.time() - t
    print("Tiempo: {:6.2f} segundos".format(tiempo))
    #Comprobar las características (variables) seleccionadas.
    
    selected_features = feat_selector.transform(X)
    ...
    X_tst = feat_selector.trandform(data_x_tmp)
    return X, X_tst
#------------------------------------------------------------------
...
lgbm, y_test_lgbm = validacion_cruzada(lgbm, X, y, skf, feat_selector)
