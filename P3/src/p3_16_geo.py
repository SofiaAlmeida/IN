...
def ajuste_lgbm(X, y, params_lgbm):
    print("------ Grid Search...")
    grid = GridSearchCV(lgbm, params_lgbm, cv=3, n_jobs=-1, verbose=2, scoring=make_scorer(f1_score, average='micro'))
    grid.fit(X, y)
...
'''
    ...
    data_x_tmp = pd.get_dummies(data=data_x, columns=['land_surface_condition', 'foundation_type',  'roof_type', 'ground_floor_type', 'other_floor_type', 'position', 'plan_configuration', 'legal_ownership_status', 'geo_level_1_id'])
    ...
    data_x_tmp = pd.get_dummies(data=data_x_tst, columns=['land_surface_condition', 'foundation_type',  'roof_type', 'ground_floor_type', 'other_floor_type', 'position', 'plan_configuration', 'legal_ownership_status', 'geo_level_1_id'])
...
print("------ LightGBM...")
lgbm = lgb.LGBMClassifier(objective='regression_l1', n_jobs=-1, n_estimators =  900, num_leaves = 70, scale_pos_weight =0.05)
