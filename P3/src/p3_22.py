...
def preprocessing(data_x, data_x_tst, data_y):
    '''
...
'''
    ...
    classifiers = {
        'Isolation Forest': IForest(contamination=outliers_fraction,random_state=123456)}
    ...
    y = data_y.loc[dfx['outlier'] == 0]
    y = np.ravel(y.values)
    
    X = (dfx.loc[dfx['outlier'] == 0]).drop(columns = 'outlier')
