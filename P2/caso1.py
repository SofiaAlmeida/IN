...
def heatmap(centers, show=False):
  ...
  #  ax.set_ylim(bottom + 0.5, top - 0.5)
    # profe: ax.set_ylim(len(centers),0)
...
def kdeplot(X, name, k, usadas, show=False):
    ...
    for i in range(k):
        ...
        for j in range(n_var):
            ...
            #sns.kdeplot(dat_filt[usadas[j]], shade = True, color = colors[i], label = "", ax = axes[0,j]) -> (en subplots cambiar k por 1) para superponerlos
            if (i==k-1):
