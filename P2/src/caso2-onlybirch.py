...
def tam_clusters(clusters, show=False):   
    ...
    fig.savefig("./fig/caso2/"+ 'Birch' + '2_tam_clusters.png', transparent=False, dpi=80, bbox_inches="tight")
    if show:
        plt.show()
...
def heatmap(centers, show=False):
    ...
    plt.savefig("./fig/caso2/" + 'Birch' + "2_heatmap.png")
    if show:
        plt.show()
    plt.clf() 
# Heatmap '''
...
def scatter_matrix(X_alg, show=False):
    print("---------- Preparando el scatter matrix " + 'Birch' + "...")
    ...
    plt.savefig("./fig/caso2/" + 'Birch' + "2_scattermatrix.png")
    if show:
        plt.show()
    plt.clf()
    print("")
...
birch = Birch(branching_factor=25, threshold=0.25, n_clusters=4)
...
cluster_predict['Birch'] = birch.fit_predict(X_normal)
tiempo = time.time() - t
#Pinto resultados
print(": {:.2f} segundos, ".format(tiempo), end='')
try:
    metric_CH = metrics.calinski_harabasz_score(X_normal, cluster_predict['Birch'])
...
try:
    metric_SC = metrics.silhouette_score(X_normal, cluster_predict['Birch'], metric='euclidean', sample_size = floor(muestra_silhoutte*len(X)), random_state=123456)
...
# Visualización
clusters = pd.DataFrame(cluster_predict['Birch'],index=X.index,columns=['cluster'])
...
centers = pd.DataFrame(getattr(birch, centroids['Birch']) ,columns=list(X))
print('Birch' + "/n", centers)
