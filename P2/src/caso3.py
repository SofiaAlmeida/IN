...
def tabla_alg(l):
    ...
    print("\\begin{table}[H]")
    print("\centering")
    print("\caption{Resultados caso de estudio 3}")
    print("\label{tab:algorithms3}")
...
def tam_clusters(clusters, show=False):   
    ...
    fig.savefig("./fig/caso3/"+ name + '_tam_clusters.png', transparent=False, dpi=80, bbox_inches="tight")
    if show:
        plt.show()
...
def bar_graph(data, etiq, show=False):   
    ...
    fig.savefig("./fig/caso3/"+ etiq + '_tam.png', transparent=False, dpi=80, bbox_inches="tight")
    if show:
        plt.show()
...
def heatmap(centers, show=False):
    ...
    plt.savefig("./fig/caso3/" + name + "_heatmap.png")
    if show:
        plt.show()
    plt.clf() 
# Heatmap '''
...
def scatter_matrix(X_alg, show=False):
    ...
    plt.savefig("./fig/caso3/" + name + "_scattermatrix.png")
    if show:
        plt.show()
    plt.clf()
    print("")
...
color = colors[i], label = "", ax = axes[i,j])
    ...
    plt.savefig("./fig/caso3/" + name + "_distplot.png")
    if show:
        plt.show()
...
def boxplot(X, name, k, usadas, show=False):
    ...
    fig.savefig("./fig/caso3/" + name + "_boxplot.png")
    if show:
        plt.show()
    plt.clf()
#'''
...
print("Tras quitar los outliers a EDADESTABLE, subconjunto formado por " + str(len(X)) + " objetos")
#outliers''' 
...
plt.savefig("./fig/caso3/Ward_dendrograma.png")
...
sns_dendro.savefig("./fig/caso3/Ward_dendro_heat.png")
